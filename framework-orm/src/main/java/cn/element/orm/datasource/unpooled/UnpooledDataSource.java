package cn.element.orm.datasource.unpooled;
...
/**
 * 对于数据库连接池的实现，不一定非得提供池化技术，
 * 对于某些场景可以只使用无池化的连接池。
 * 那么在实现的过程中，可以把无池化的实现和池化实现拆分解耦，在需要的时候只需要配置对应的数据源即可。
 * 
 * 无池化的数据源链接实现比较简单，
 * 核心在于 initializerDriver 初始化驱动中使用了 Class.forName 和 newInstance 的方式创建了数据源链接操作。
 * 
